...
public sealed class OracleDrawingStore : IDrawingStore
{
    ...
    public async Task<SearchResult> SearchAsync(SearchParams p, CancellationToken ct = default)
    {
        ...
        await using var con = Open();
        // CommandDefinition carries the CancellationToken: when the browser aborts the
        // request (filters changed again, page left) Oracle stops the query too.
        var total = await con.ExecuteScalarAsync<int>(
            new CommandDefinition($"select count(*) {BaseSelect} {wh}", args, cancellationToken: ct));
        var items = (await con.QueryAsync<DrawingRow>(new CommandDefinition(
            $@"select {Cols} {BaseSelect} {wh}
               order by {orderBy}
               offset :off rows fetch next :n rows only", args, cancellationToken: ct))).ToList();
